...
def train(
    ...
    data_list = load_data(data_path, add_noise=add_noise)
    ...
    if normalize:
        ...
        import joblib
        joblib.dump((feature_scaler, indices), os.path.join(output_path, "scaler_soh.pkl")) 
    

    # 滑窗处理
    #temporal = TemporalFeatureWrapper(window_size=5)
    #features = temporal.transform(features)
    print(features[0])
    ...
    # 构建预测模型
    model = SOHPredictor(input_dim).to(device)
    reconstruction_criterion = ModuleAwareLoss(alpha=0.3, device=device)#CustomLoss()#
    ...
    # model, optimizer, train_inputs, val_inputs = accelerator.prepare(
    #     model, optimizer, train_inputs, val_inputs
    # )
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, len(train_features), batch_size):
            ...
            recon_loss = reconstruction_criterion(reconstructed, batch_inputs)
        ...
        with torch.no_grad():
            for i in range(0, len(val_features), batch_size):
                ...
                val_loss += total_loss.item() * len(batch_inputs)
        ...
        if val_loss < best_val_loss:
            ...
            no_improve = 0
            # 保存最佳模型
            torch.save(model.state_dict(), os.path.join(output_path, "best_sohmodel.pth"))
        # else:
        #     no_improve += 1
    ...
    # 保存模型
    os.makedirs(output_path, exist_ok=True) 
    torch.save(model.state_dict(), os.path.join(output_path, "soh_model.pth"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume_from_checkpoint", type=str, default="../outputs/best_sohmodel.pth", help="从检查点恢复训练")
