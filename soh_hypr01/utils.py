...
class SOHDetector:
    ...
    def warning(self, soh, data_soh, data_cls, key_map=None):
        ...
        if self.remembered_soh[-1] + self.remembered_num * slope < self.threshold * 100:
...
def Motor_rolling_window_features(data_list, window_size=3):
    ...
    # 获取舵机数据和其他特征数据
    gps_data = data_array[:, 0:24]  # 前24列是舵机数据
...
def load_data(data_path, add_noise=False, preprocess=True):
    """
    加载数据集
    """
    ...
    data_pd = pd.read_csv(data_path, header=None, sep=" ", names=columns)
